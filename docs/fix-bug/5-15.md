...
### 上层用法流程图（WalletKitProvider - 开箱即用）

这是最简单的用法，你只需要用 `WalletProvider`（即 `WalletKitProvider`），所有 Provider 都帮你包好了：

```mermaid
graph TB
    subgraph "你的应用"
        App[你的 App<br/>业务代码]
        Hooks1[snk-wallet-kit hooks<br/>useConnectWallet / useCurrentAccount...]
        Hooks2[wagmi hooks<br/>useAccount / useWriteContract...]
        Hooks3[react-query hooks<br/>useQuery / useMutation...]
    end

    subgraph "WalletKitProvider（帮你包好了）"
        direction TB
        QC[创建 QueryClient]
        WC[调用 createEvmConfig]
        QCP[QueryClientProvider]
        WP[WagmiProvider]
        WCP[WalletCoreProvider]
        WCtx[Wallet Context<br/>连接状态/当前钱包]
    end
...
这是高级用法，你需要自己管理 `QueryClientProvider` 和 `WagmiProvider`，然后把它们传给 `WalletCoreProvider`：
...
graph TB
    ...
    subgraph "你的自定义 Provider 层级"
        QCP[QueryClientProvider<br/>（你自己的）]
        WP[WagmiProvider<br/>（你自己的）]
        WCP[WalletCoreProvider]
        WCtx[Wallet Context<br/>连接状态/当前钱包]
    end
